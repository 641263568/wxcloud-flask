...
async function call(name, data) {
  ...
  if (token.token == null || token.ca != null) {
    // 本地调试时不存在token，开启接口服务proxy后使用免token模式
    ssl = false;
  }
  // 包装api请求
  var options = {
    method: "POST",
    url: `${ssl === true ? "https" : "http"}://api.weixin.qq.com/${name}${
      ssl === true ? "?cloudbase_access_token=" + token.token : ""
    }`,
    headers: {
  ...
  console.log(
    ssl === true ? "发起https请求，带token" : "发起http请求，免token"
  );
  return new Promise((resolve, reject) => {
    request(options, async function (error, response) {
      if (error) {
        ...
        console.log(
          `${seqid != null ? "开放服务调用｜" + seqid : "AccessToken调用"}`
        );
        ...
        try {
          ...
          if (response.body == null || response.body === "") {
            console.log(
              "本地调试时，需开启接口服务proxy后再试！传统服务器请使用原生微信token！教学戳此：https://www.bilibili.com/video/BV1QS4y1S7C7"
            );
            resolve({
              // 返回网络问题
              errcode: -1,
              errmsg:
                "请求错误！本地调试时，需开启接口服务proxy后再试！传统服务器请使用原生微信token！",
            });
          } else if (response.body.indexOf("establish") !== -1) {
            console.log(
              "你的环境服务没有配置出网NAT网关，请前往配置，链接：https://console.cloud.tencent.com/vpc/vpc"
            );
            resolve({
              // 返回网络问题
              errcode: -1,
